const sBox = new Uint8Array([
  
]);

const rCon = new Uint8Array([0x00, 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36]);

const mul2 = (value) => {
  const shifted = value << 1;
  return ((shifted & 0xff) ^ ((value & 0x80) ? 0x1b : 0)) & 0xff;
};

const mul3 = (value) => mul2(value) ^ value;

const subBytes = (state) => {
  for (let i = 0; i < 16; i++) {
    state[i] = sBox[state[i]];
  }
};

const shiftRows = (state) => {
  let tmp = state[1];
  state[1] = state[5];
  state[5] = state[9];
  state[9] = state[13];
  state[13] = tmp;

  tmp = state[2];
  state[2] = state[10];
  state[10] = tmp;
  tmp = state[6];
  state[6] = state[14];
  state[14] = tmp;

  let tmp1 = state[3];
  let tmp2 = state[7];
  let tmp3 = state[11];
  state[3] = state[15];
  state[7] = tmp1;
  state[11] = tmp2;
  state[15] = tmp3;
};

const mixColumns = (state) => {
  for (let c = 0; c < 4; c++) {
    const i = c * 4;
    const a0 = state[i];
    const a1 = state[i + 1];
    const a2 = state[i + 2];
    const a3 = state[i + 3];

    state[i] = (mul2(a0) ^ mul3(a1) ^ a2 ^ a3) & 0xff;
    state[i + 1] = (a0 ^ mul2(a1) ^ mul3(a2) ^ a3) & 0xff;
    state[i + 2] = (a0 ^ a1 ^ mul2(a2) ^ mul3(a3)) & 0xff;
    state[i + 3] = (mul3(a0) ^ a1 ^ a2 ^ mul2(a3)) & 0xff;
  }
};

const addRoundKey = (state, roundKeys, offset) => {
  for (let i = 0; i < 16; i++) {
    state[i] ^= roundKeys[offset + i];
  }
};

const expandKey = (key) => {
  const roundKeys = new Uint8Array(16 * 11);
  roundKeys.set(key);

  const temp = new Uint8Array(4);
  let bytesGenerated = 16;
  let rconIter = 1;

  while (bytesGenerated < roundKeys.length) {
    temp[0] = roundKeys[bytesGenerated - 4];
    temp[1] = roundKeys[bytesGenerated - 3];
    temp[2] = roundKeys[bytesGenerated - 2];
    temp[3] = roundKeys[bytesGenerated - 1];

    if (bytesGenerated % 16 === 0) {
      const t = temp[0];
      temp[0] = temp[1];
      temp[1] = temp[2];
      temp[2] = temp[3];
      temp[3] = t;

      for (let i = 0; i < 4; i++) {
        temp[i] = sBox[temp[i]];
      }

      temp[0] ^= rCon[rconIter];
      rconIter++;
    }

    for (let i = 0; i < 4; i++) {
      roundKeys[bytesGenerated] = roundKeys[bytesGenerated - 16] ^ temp[i];
      bytesGenerated++;
    }
  }

  return roundKeys;
};

export class Aes128 {
  constructor(keyBytes) {
    if (!(keyBytes instanceof Uint8Array) || keyBytes.length !== 16) {
      throw new Error('AES-128 key must be a 16-byte Uint8Array');
    }
    this.roundKeys = expandKey(keyBytes);
  }

  encryptBlock(block, out = new Uint8Array(16)) {
    if (!(block instanceof Uint8Array) || block.length !== 16) {
      throw new Error('AES-128 block must be a 16-byte Uint8Array');
    }
    if (!(out instanceof Uint8Array) || out.length !== 16) {
      throw new Error('Output buffer must be a 16-byte Uint8Array');
    }

    out.set(block);
    addRoundKey(out, this.roundKeys, 0);

    for (let round = 1; round < 10; round++) {
      subBytes(out);
      shiftRows(out);
      mixColumns(out);
      addRoundKey(out, this.roundKeys, round * 16);
    }

    subBytes(out);
    shiftRows(out);
    addRoundKey(out, this.roundKeys, 10 * 16);

    return out;
  }
}
